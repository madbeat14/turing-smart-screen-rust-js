...
function compileCss(manifest) {
  // Collect unique styles from all widgets
  var firstStyle = manifest.widgets.length > 0 ? manifest.widgets[0].style : DEFAULT_STYLE;
  var s = firstStyle || DEFAULT_STYLE;
  ...
  css.push('.monitor { position: relative; width: 100%; height: 100%; }');
  css.push('');
...
function compileJs(manifest) {
  ...
  js.push('  } catch (e) { /* prevent sensor errors from crashing render */ }');
  js.push('}');
  js.push('');
...
function compileMetricCardUpdateJs(w) {
  ...
  lines.push('  if (pVal != null) setValueInElement("primary-' + w.id + '", pVal.toFixed(0));');

  // Secondary fields
  if (c.secondaryFields) {
    for (var i = 0; i < c.secondaryFields.length; i++) {
      var sf = c.secondaryFields[i];
      lines.push('  if (data.' + sf.field + ' != null) setText("sec-' + i + '-' + w.id + '", data.' + sf.field + '.toFixed(0));');
    }
  }
  ...
  lines.push('  var state = ' + threshFn + '(pVal);');
  lines.push('  applyState("card-' + w.id + '", state);');

  // Progress bar
  if (c.showProgress && c.progressField) {
    lines.push('  var barVal = data.' + c.progressField + ';');
  ...
  if (c.showSparkline && c.sparklineField) {
    lines.push('  var sparkVal = data.' + c.sparklineField + ';');
